...
function init() {
  ...
  // Call the searchPlacesAmenities function to fetch and display places data based on selected amenities
  PlacesSearchAmenities();
}
...
// Function to fetch places data based on selected amenities from the API and display it on the webpage
function PlacesSearchAmenities() {
  // The API endpoint URL for fetching places data
  const PLACES_URL = `http://${HOST}:5001/api/v1/places_search/`;
  
  // Send a POST request to the API to get the places data based on selected amenities
  $.ajax({
    url: PLACES_URL,
    type: 'POST',
